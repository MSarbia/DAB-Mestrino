        ...
        (function (common) {
            ...
            (function (widgets) {
                ...
                (function (sidebar) {
                    ...
                    *
                    */
                    var SidebarDOMService = (function () {
...
(function () {
    ...
    *
    * ```
    *   A JSON object provides data to the **sidebar** from the **Config** file. We must define a **constant** with a name **CONFIG** within a **Config** file that contains the following properties:
    ...
    *              Type: **string**
    *   * **icon**: used to display an Icon for each of the button items. Value should be a valid [FontAwesome](http://fortawesome.github.io/Font-Awesome/icons/) CSS class that determines the node icon to be displayed. If not specified, default Icon will be displayed.  
    *               Type: **string**
    ...
    *               Type: **Object[]**
    *   * **actions**: used to configure events. `click` is an event can be configured to the public methods in the `actions` object. `toggle`, `navigate`, `navigateBack` are the available methods to configure 'click' action.
    *               Type: **string**
    *   * **bind**: used to configure properties. `display` is a property can be configured to the public properties in the `options` object. 'hasBackNavigation' is the available property to configure 'bind' property.
    *               Type: **string**
    * 
    * 
  ...
  */
    ...
    function sitSidebar() {
        ...
        function SidebarController($rootScope, $scope, $state, $translate, $log,
            ...
            //TODO : this code is for calculating the css expression 
            function _calc() {
        ...
        (function (common) {
            ...
            (function (widgets) {
                ...
                (function (sidebar) {
                    ...
                    var SidebarMenuItem = (function () {
                        function SidebarMenuItem(id, title, icon) {
                            this.id = id;
                            this.title = title;
                    ...
                    var SidebarService = (function () {
                        function SidebarService(CONFIG, $log, $rootScope) {
                            this.CONFIG = CONFIG;
                            this.$log = $log;
                            this.$rootScope = $rootScope;
                            this.init();
                            ...
                            for (var i = 0; i < source.length; i++) {
                                ...
                                if (item.contents && item.contents.length > 0) {
                                    for (var j = 0; j < item.contents.length; j++) {
                                        var subItem = item.contents[j];
                                        if (subItem.id === id) {
                                            homeItem = new SidebarMenuItem(subItem.id, subItem.title, subItem.icon);
                                            itemFound = true;
                                        }
                                        if (subItem.contents && subItem.contents.length > 0 && itemFound === false) {
                                            for (var k = 0; k < subItem.contents.length; k++) {
                                                if (subItem.contents[k].id === id) {
                                                    homeItem = new SidebarMenuItem(subItem.contents[k].id, subItem.contents[k].title, subItem.contents[k].icon);
                                                    itemFound = true;
                                                }
                                            }
                                        }
                        ...
                        SidebarService.prototype.validateItem = function (menuItem) {
                            if (menuItem.id === undefined) {
                                ...
                                var item = new SidebarMenuItem(menuItem.id, menuItem.title, menuItem.icon);
                    ...
                    SidebarService.$inject = ['CONFIG', '$log', '$rootScope'];
