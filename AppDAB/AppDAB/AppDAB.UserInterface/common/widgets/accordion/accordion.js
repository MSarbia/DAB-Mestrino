...
(function () {
    ...
    * @property {Function} onClickCallback A function to execute when the menu item is clicked. The function is called with one parameter, corresponding to the item that was clicked.
    ...
    * @property {Function} onClickCallback A function to execute when any menu item is clicked. The function is called with one parameter, corresponding to the item that was clicked.  
    *
    * @example
    * ```
    *    {
    ...
    * @description Refreshes the widget, re-applying settings to all configured items. It is necessary to explicitly call this method after updating a {@link MenuItem} object directly, to reflect the changes in the user interface. 
    **/
                    ...
                    _.each(objToCompile, function (item, index) {
                        ...
                        if (item.items && !(_.isEmpty(item.items))) {
                            ...
                            elements += '<div data-internal-type="accordion-item_' + level + '' + index + '" id="' + item.id + '" ng-show="' + isVisible + '" class="sit-panel-item sit-panel-cont  background-level-common item-space-' + level +
                                ...
                                '<div class="sit-panel-item-img fa ' + ((item.icon) ? item.icon : vm.config.icon) + ' icon-level-' + (level) + ' "></div>' +
