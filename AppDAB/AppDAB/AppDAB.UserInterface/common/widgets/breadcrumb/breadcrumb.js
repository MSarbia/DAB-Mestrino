...
(function () {
            ...
            link: {
                post: function postLink(scope, element, attrs, ctrl) {
                    ...
                    function showFlyoutMenu() {
                        ...
                        //dots.css({ 'border': '1pt solid Black', 'border-bottom': 'none' });
                        ctrl.setFlyoutType();
    ...
    function breadcrumbController($interpolate, breadcrumbService, $filter) {
        ...
        function draw() {
            //var viewScope = breadcrumbService.getLastViewScope();
     ...
     *   @restrict E
     *   @description 
     *   Breadcrumb bar provides links or trails to previous pages navigated by the user in a hierarchical site structure. This helps the user to follow back to the starting or entry point. 
     ...
     *   * **Minimized mode:** If the available space for the breadcrumb cannot contain the entire breadcrumb, first item and last item in the breadcrumb is shown. An ellipsis (...) shown between two items indicates that the other items are available. When the user clicks on the ellipsis, all the hidden items are shown in an overlay fashion using a flyout (a new breadcrumb below the original breadcrumb).
     *      If the available space in the flyout cannot display all the items, a flyout menu is used to show the remaining items.  
     *   * **Compact mode:** If the space available is not enough to contain first item, last item, and the ellipsis, then ellipsis and the last item are shown. A flyout menu with remaining items are shown when user clicks on the ellipsis.
     ...
     *   * **data**: specifies the custom data associated with the current state. The **ViewDetailCtrl** controller of **mashupEditor.views** state contains the title of the current state that appears when the breadcrumb is displayed. 
     ...
     * });
     * ```
     * In the above example the breadcrumb title for the **breadCrumbTest.views.detail** state is dynamically set by using the setTitle method of the breadcrumb service(common.breadcrumb.breadcrumbService).
