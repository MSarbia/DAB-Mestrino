...
(function () {
    ...
    *   @param {Object} sit-templateuri _(Optional)_ Path to the custom template with the content to render within the dialog. If not specified, the default template (containing a {@link sitPropertyGrid sit-property-grid} directive configurable via the **sit-templatedata** property) will be used.
    *
    ...
    *   * **data**: An array of objects used to configure the fields of the sit-property-grid directive . For more information, see the **sit-data** attribute of the {@link sitPropertyGrid sit-property-grid} directive.
    *
    *
    *   If a custom template is specified, there are no restrictions on the format of the object, and, within the custom template, it can be accessed via the **Dialog.templatedata** variable.
    ...
    * ```
    *
    * If a custom template is not provided, the dialog height will be set automatically depending on the height of the content provided. If the content exceeds the viewport height, the container of the content will scroll and the dialog will be set to the height of the viewport.
    *
    * If a custom template is provided, you can customize the template by providing css styles as recommended in the example below. If no styles are specified, the default template styles will be applied.
    *
    * ```
    ...
    *
    */
    app.directive('sitDialog', [function () {
        return {
            bindToController: {
                title: "=sitTitle",
            ...
            controller: ['$scope', '$state', '$window', 'common.widgets.dialog.service', 'common', 'LOG_TYPES', DialogController],

            controllerAs: 'Dialog'
        };
    }]);

    function DialogController($scope, $state, $window, dialogService, common, LOG_TYPES) {
        var vm = this;
        var modalid;
        //logs
        ...
        var propGridValHandle = $scope.$on('sit-property-grid.validity-changed', updateValidity);
...
(function () {
    ...
    */
    ...
    DialogService.$inject = [ 'LOG_TYPES', '$window'];
    function DialogService(LOG_TYPES, $window) {
        ...
        // var logError = logger.getLogFn('Dialog Service', LOG_TYPES.LOG_ERROR);
        ...
        */
        ...
        this.showDialogModal = function (id, posId) {
            ...
            function setDimensionOnResize() {
                var dialogBodyDimensions = {
                    maxHeight: popupEl[0].offsetHeight - headerFooterHeight,
                    maxWidth: popupEl[0].offsetWidth - modalMargin
                };
                popupEl.find('.dialog-body-overflow').css(dialogBodyDimensions);
            }
        ...
        this.hideDialogModal = function (id) {
            ...
            if (popupEl.length === 0) {
                //logError('The popup cannot be hidden because its id does not exist \n', { id: id });
                return;
            }
