...
(function () {
    ...
    function settingsCtrl($rootScope, $scope, $filter, CONFIG, APPCONFIG, common, notificationTileService, LOG_LEVELS, loggerConfig, $modal, RESOURCE, THEMES) {
        ...
        //init languages
        sc.languages = [];
        //var identity = common.authentication.notifyUser();
        ...
        //if (identity && identity.language && config.languages.indexOf(identity.language) > -1) {
        //    sc.defaultLanguage = identity.language;
        //    sc.disableDropdown = true;
        ...
        sc.languageSelected = function (itemLanguage) {
            ...
            //sc.languages.forEach(function(item, idx){
            //    item.selected = (item.value === sc.currentLanguage);
            //});
        ...
        sc.restoreSettings = function () {
            /*var settings = common.loadSettings();
            if (settings){
                common.globalization.getLocale(settings.lang_key);
                $rootScope.currentTheme = settings.theme;
    ...
    unityApp.controller('settingsCtrl', ['$rootScope', '$scope', '$filter', 'CONFIG', 'APPCONFIG', 'common', 'common.widgets.notificationTile.service', 'LOG_LEVELS', 'common.services.logger.config', '$uibModal', 'RESOURCE', 'THEMES', settingsCtrl]);
