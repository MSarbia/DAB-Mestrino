...
(function () {
    ...
    function ($translateProvider, $translatePartialLoaderProvider) {
         ...
         * @description  Registers a JSON resource file path.
          * @param {Object[]} parts An array of objects, each containing a property "name" that corresponds to the resource part identifier of the JSON file name used for translations.(example: if the file is named custom-widget.it-IT.json, the property name will be custom-widget).
    ...
    function globalizationService($rootScope, $http, $q, $translate, $cacheFactory, globalizationConfig, runtimelocale, logger, CONFIG, APPCONFIG, LOG_LEVELS) {
        ...
        */
         ...
         * In order to list the locales distributed by the UI Framework, the common/scripts/i18n folder should contain a locales.json file, which contains the name of the languages.         
         *
         */
        function getSupportedLocales() {
            return JSON.parse(window.sessionStorage.getItem("supportedLocales"));
        }
        ...
        function setResourcePath(path, parts) {
            ...
            if (enableGlobalizationDebug) {
                parts.forEach(function (part) {
                    promises.push(checkPart(path, part));
                });
