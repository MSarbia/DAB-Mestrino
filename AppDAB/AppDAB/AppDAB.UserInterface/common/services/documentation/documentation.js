...
(function () {
    ...
    function documentationCenterService($q, $resource, $http, $translate, common, CommandResponse, ExecutionError, documentationCenterConfigProvider) {
        ...
        function removeMetafromArrayJSON(data) {
            ...
            var metaToRemove = new Array(["BookletId", "Author", "BookletSummary", "State", "Revision", "ReleaseId", "ProductOptionVersions", "Product", "Version", "MasterProductVersion", "Language"]);
        ...
        function setHightlightTag(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].Contents) {
                    ...
                    //strHtml = strHtml.replace(new RegExp('<code>'.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'), '<code sit-code-highlight>');
        ...
        function getExpandedPage(id) {
            var deferred = $q.defer();
            var path = oDataPath + "Page('" + id + "')?$expand=Document";
        ...
        function getFirstPageFromDoc(id) {
            var deferred = $q.defer();
            //var path = oDataPath + "Page?$filter=DocumentId eq '" + id + "'&$expand=Document";
            var path = oDataPath + "Page?$filter=DocumentId eq '" + id + "'&$expand=Document&$top=1";
            if (!id || id === '') {
                deferred.reject({
                    data: new CommandResponse(false, new ExecutionError(-1, $translate.instant('svcErrors.documentation.noID')))
                });
